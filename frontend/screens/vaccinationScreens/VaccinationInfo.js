...
const VaccinationInfo = ({ route, navigation }) => {
  ...
  const handleConfirmDelete = () => {
    ...
    getToken().then(async (token) => {
      response = await fetch(context.fetchPath + 'api/deleteVaccine', {
        method: 'DELETE',
        headers: {
      ...
      if (json.message === 'Deleted vaccine') {
        Toast.show({
          text1: `Success!`,
          text2: `Successfully deleted vaccination record: ${vaccineInfo.agent}`,
          type: 'success',
        });
